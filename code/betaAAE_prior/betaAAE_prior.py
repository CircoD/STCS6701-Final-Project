...
def make_fake_latent_vector(latent_vector):
    fake_latent_vector = torch.randn_like(latent_vector)
    return fake_latent_vector
...
def run(config_name):
    ...
    with torch.autograd.set_detect_anomaly(True):
        for epoch in range(Epochs):
            ...
            for batch_idx, (Images, Labels) in enumerate(train_loader):
                Images, Labels = Images.to(device), Labels.to(device)
                decoded_images, latent_vector = autoencoder(Images)
                fake_latent_vector = make_fake_latent_vector(latent_vector)
    ...
    autoencoder.save('betaAAE_prior_beta={}.pth'.format(Beta))

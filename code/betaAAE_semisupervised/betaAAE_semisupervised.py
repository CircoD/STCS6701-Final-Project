...
def fake_latent_vector(latent_vector, one_hot_labels):
...
def run(config_name):
    ...
    discriminator = Discriminator(latent_dim=model_config['latent_dim']+model_config['num_classes']).to(device)
    ...
    with torch.autograd.set_detect_anomaly(True):
        for epoch in range(Epochs):
            ...
            for batch_idx, (Images, Labels) in enumerate(train_loader):
                ...
                one_hot_labels = F.one_hot(Labels.to(torch.int64), num_classes=autoencoder.num_classes).float()
                decoded_images, latent_vector = autoencoder(Images, Labels)
                reconstruction_loss = F.mse_loss(decoded_images, Images, reduction='sum')
                labelled_latent_vector = torch.cat([latent_vector, one_hot_labels], dim=1)
                discriminator_validity = discriminator(labelled_latent_vector)
                validity = torch.ones_like(discriminator_validity)
                Validity_loss = F.binary_cross_entropy(discriminator_validity, validity, reduction='sum')
            ...
            for batch_idx, (Images, Labels) in enumerate(train_loader):
                ...
                decoded_images,latent_vector = autoencoder(Images, Labels)
                labelled_latent_vector = torch.cat([latent_vector, one_hot_labels], dim=1)
                labelled_fake_latent_vector = fake_latent_vector(latent_vector, one_hot_labels)
                real_discr_output = discriminator(labelled_latent_vector)
                fake_discr_output = discriminator(labelled_fake_latent_vector)
    ...
    autoencoder.save('betaAAE_semisupervised_beta={}.pth'.format(Beta))

    for test_x, test_y in test_loader:
        test_x = test_x.to(device)
        test_y = test_y.to(device)

    img_gen, z = autoencoder(Images = test_x, Labels = test_y)
